    ...
    messages = [
        {
            ...
            "content": (
                ...
                "• If the user asks to echo text and supplies a Kerberos token, "
                "call the `echo` tool.\n"
                "• If the user asks which auth methods a person can use, "
                "call the `get_auth_method` tool."
            ),
        },
...
async def access_mcp_tools():
    async with streamablehttp_client(MCP_ENDPOINT) as (r, w, _):
        async with ClientSession(r, w) as session:
            ...
            print("Examples:")
            print("  - What auth methods can jane.doe use?")
            print("  - Echo 'hello' using Kerberos token mock-valid-token")
